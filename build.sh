...
# The analysis modules alongside it (roundtrip_fram, supersession_check, rename,
# ...) are graph-upstream too; each emits to out/<module>.clj, so
# `bb -cp out -m <ns> ...` runs it.
for m in codegraph roundtrip_fram supersession_check rename; do
  BEAGLE_EMIT_SRCLOC=0 direnv exec "$BEAGLE" "$BEAGLE/bin/beagle-build" \
