;; server_authority_snapshot_test.clj — the live sealed-runtime authority descriptor
...
;; the wrapper-sealed FRAM_* bindings) and the LIVE server store — runtime.closureDigest is
...
;;
;;   bb -cp out tests/server_authority_snapshot_test.clj
;; Scratch /tmp only; never a live server, no socket, no North.
...
;; a live server seeded with two module @<mod>#root `file` facts.
(defn live-database-with-modules [code-log]
  (let [db (new-database code-log)]
    (register-pred! db "file" "single" "literal")
    (commit! db "database" "@empty#root" "file" :assert "empty.bclj" (current-seq db))
    (commit! db "database" "@src.fram.authority#root" "file" :assert "src/fram/authority.bclj" (current-seq db))
    db))
...
(defn base-inputs [db manifest checkout source]
...
(defn seal-of [db inputs]
  (let [ctx (authority-context db (fresh-instance-id))
...
(let [{:keys [checkout source code-log]} (make-checkout!)
      db (live-database-with-modules code-log)
      manifest (write-manifest! {})
      inputs (base-inputs db manifest checkout source)
      snap (derive-authority-snapshot inputs)
      sealed (seal-of db inputs)]
...
(let [{:keys [checkout source code-log]} (make-checkout!)
      ...
      manifest (write-manifest! {})
      inputs (base-inputs db manifest checkout source)
      handle (:handle (authority-session-open (authority-context db (fresh-instance-id))
                                              "R" "client-a" spki-a 60000))
      ctx (authority-context db "instance-fixed")
...
(let [{:keys [checkout source code-log]} (make-checkout!)
      db (live-database-with-modules code-log)
      good (write-manifest! {})
      ok-inputs (base-inputs db good checkout source)]
  ...
  (check "negative: a well-formed-but-wrong-length North digest cannot seal"
         (throws? #(seal-of db (assoc ok-inputs :closure-digest "sha256:abc"))))
  ...
  (let [co2 (new-database (scratch ".log"))]
    (register-pred! co2 "file" "single" "literal")
    (commit! co2 "database" "@weird#root" "file" :assert "weird.txt" (current-seq co2))  ; non-Beagle extension
    (check "negative: a module file fact without a Beagle extension fails closed"
           (throws? #(derive-authority-snapshot (assoc ok-inputs :db co2)))))
  (let [co3 (new-database (scratch ".log"))]
    (register-pred! co3 "file" "single" "literal")
    (commit! co3 "database" "@wrong#root" "file" :assert "valid.bclj" (current-seq co3))
    (check "negative: module root identity/path disagreement fails closed"
           (throws? #(derive-authority-snapshot (assoc ok-inputs :db co3)))))
...
;; NO server or North contact when a launch binding is missing.
;; ===========================================================================
(let [{:keys [code-log]} (make-checkout!)
      db (live-database-with-modules code-log)]
  (check "darkness: authority-launch-snapshot fails closed on a missing launch binding"
         (throws? #(authority-launch-snapshot db))))  ; test env supplies no sealed authority bindings

(println (str "\nserver-authority-snapshot: " @pass " / " (+ @pass @fail) " PASS"))
