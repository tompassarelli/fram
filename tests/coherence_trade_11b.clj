;; ============================================================================
;; coherence_trade_11b.clj — #11b COUPLED-COHERENCE TRADE RECEIPT.  *** GIT WINS. ***
;; ----------------------------------------------------------------------------
...
;;        base_version OCC + depends_on/part_of acyclicity, database.clj:118-153) AND no
...
;; ============================================================================
;; FRAM ARM — real commit!/retract! OCC wire on an ISOLATED temp database.
;; (NEVER the canonical north log — a fresh new-database on a /tmp path.)
;; ============================================================================
(defn fresh-database []
  (let [log (str "/tmp/store-11b-" (System/nanoTime) ".log")
        db (new-database log)]
    (register-pred! db "depends_on" "multi" "ref")     ; ref edge, acyclicity-gated
    (register-pred! db "title" "single" "literal")      ; a thread is "defined" iff it has a title
    [db log]))
...
(defn fram-V [db]
  (let [st (store db)
        ...
        edges (when (and foo dep) (map #(:r (c/fact-of st %)) (live-cids-lp db foo dep)))
        defined? (fn [node] (boolean (seq (live-cids-lp db node title))))
        dangling (vec (remove defined? edges))]
    {:coherent? (empty? dangling) :foo-edges (count edges) :dangling-targets (count dangling)}))

(defn fram-setup! [db]
  (commit! db "init" "foo" "title" :assert "Foo" 0)        ; foo defined
  (commit! db "init" "bar" "title" :assert "Bar" 0))       ; bar defined

(defn fram-edit-A! [db] (commit! db "agentA" "foo" "depends_on" :link "bar" 0))  ; foo -> bar
(defn fram-edit-B! [db]                                                          ; remove bar (drop its title)
  (let [bar (s/resolve-name (store db) "bar")
        base (base-version db bar (c/value-id (store db) "title"))]
    (retract! db "agentB" "bar" "title" "Bar" base)))

;; --- A alone (coherent), B alone (coherent), then A+B combined ---
(def A-only (let [[db _] (fresh-database)] (fram-setup! db) (fram-edit-A! db) (fram-V db)))
(def B-only (let [[db _] (fresh-database)] (fram-setup! db) (fram-edit-B! db) (fram-V db)))

(let [[db _] (fresh-database)]
  (fram-setup! db)
  (def rA (fram-edit-A! db))                ; Edit A commits
  (def rB (fram-edit-B! db))               ; Edit B commits, disjoint (te=bar,p=title) vs (te=foo,p=depends_on)
  (def fram-occ (count (filter #(= :conflict (:reject %)) [rA rB])))
  (def fram-after (fram-V db)))
