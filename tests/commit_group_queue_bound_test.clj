;; Run: bb -cp out tests/commit_group_queue_bound_test.clj
...
(def script
  '(do
     ...
     (let [dir (.toFile
           ...
           fourth (atom nil)
           blocked-status
           (locking database/group-io-lock
             (binding [database/*durable-tickets* tickets]
               (database/enqueue-durable! path ["line-0\n"] nil)
               (loop [remaining 100]
                 (when (and (pos? (database/durable-queue-depth))
                            (pos? remaining))
                   (Thread/sleep 10)
                   (recur (dec remaining))))
               (doseq [i [1 2]]
                 (database/enqueue-durable!
                  path [(str "line-" i "\n")] nil)))
             (reset! fourth
                     (future
                       (binding [database/*durable-tickets* tickets]
             ...
             {:blocked (not (realized? @fourth))
              :status (database/durable-queue-status)})]
...
(let [env (assoc (into {} (System/getenv)) "FRAM_GROUP_QUEUE" "2")
      ...
      pass? (and result
                 ...
                 (= 4 (:line-count result)))]
  (if pass?
    (println "database-group-queue-bound: all checks passed" (pr-str result))
    (do
      (binding [*out* *err*]
        (println "database-group-queue-bound: FAILED" (pr-str result))
        (println (:err probe)))
      (System/exit 1))))
