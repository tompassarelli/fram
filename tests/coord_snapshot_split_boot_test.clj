...
(write-snapshot! @co coordination)
...
(check! "boot-both-ways verification diff is empty"
        (= {:ok true
            ...
            :fold-version (current-seq @co)}
           (:verification @last-boot)))
(check! "snapshot+tails equals an independent full merged fold"
        (:ok (snapshot-reconcile @co coordination)))

;; Keep the coordination identity and offsets intact but replace telemetry
...
(let [failures (remove second @checks)]
  (println)
  (if (empty? failures)
    (println "coord_snapshot_split_boot_test:" (count @checks) "/" (count @checks) "PASS")
    (do
      (println "coord_snapshot_split_boot_test:" (count failures) "FAILED")
      (System/exit 1))))
