;; snapshot_boot_test.clj — thread 019f2190 snapshot boot gate.
;; snapshot_test.clj proves the incremental reconstruction itself; THIS file proves
...
;; Run from the repo root: bb -cp out tests/snapshot_boot_test.clj
...
;; North telemetry half captured by snapshot_daemon at load time.
...
(defn ground-truth [] (live-name-triples (migrate-flat->database LOG)))
...
(let [checks (atom [])
  ...
  (chk "snapshot boot: state == whole-migrate ground truth" (= (live-name-triples @db) truth))
  (chk "snapshot boot: version == unfiltered fold version (torn tail counted)"
       (= (current-seq @db) truth-version))
  (chk "snapshot boot: reconcile gate green" (:ok (snapshot-reconcile @db LOG)))
  (let [st (:store @db) t1 (s/resolve-name st "@T1")]
  ...
  (chk "flag OFF: state still correct" (= (live-name-triples @db) truth))
  ...
  (chk "stale fold_version: state correct via fold" (= (live-name-triples @db) truth))
  ...
  (chk "reset log: state == fold of the NEW log (never the stale checkpoint)"
       (= (live-name-triples @db) (ground-truth)))
  (spit LOG saved-log)
  ...
  (chk "truncated log: state == fold of what remains" (= (live-name-triples @db) (ground-truth)))
  (spit LOG saved-log)
  ...
  (chk "torn sidecar: state correct via fold" (= (live-name-triples @db) truth))

  ;; --- (7) torn IMAGE: hash gate rejects, fallback ---
  (write-snapshot! @db LOG)                   ; fresh good checkpoint (log grew @snapshot:* facts)
  ...
  (chk "torn image: state correct via fold" (= (live-name-triples @db) truth2))
  ...
  (chk "no sidecar: state correct via fold" (= (live-name-triples @db) truth2))
