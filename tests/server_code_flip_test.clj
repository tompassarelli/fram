;; ============================================================================
;; server_code_flip_test.clj — THE FLIP gate (CODE source-of-truth demotion).
;; ============================================================================
;; Sibling of server_flip_test.clj (which is the THREAD cutover, NOT the code flip).
...
;; trusted). NEVER touches port 7977 or the north log.
;;
...
;;               after a set-body delta committed THROUGH the server,
...
;;               server and reads back — proof the code + thread frames compose.
...
;;
;;   bb -cp out server_code_flip_test.clj      (run from the fram repo root)
...
;; --- load the daemon machinery + resolver IN-PROCESS (server_dropin_test pattern) --
...
(chk "SANITY: daemon :status :log == our /tmp code-log copy (not 7977/north)" status-ok)
(when-not status-ok
...
;; ===========================================================================
;; GATE 4 — CROSS-FRAME: a bridge fact composes through the ONE server.
;; ===========================================================================
...
;; K-C: COMMIT-THE-DELTA — a set-body edit committed THROUGH the server, then
...
;; commit the delta THROUGH this daemon's server (the code log).
(def kc-commit
