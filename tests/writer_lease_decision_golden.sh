...
run_case lease-decisions bb -cp out tests/writer_lease_decision_golden.clj
...
case "$mode" in
  capture)
    ...
    echo "writer_lease_decision_golden: captured original lease + cascade scenarios -> $golden_dir"
    ;;
  verify)
    diff -ru "$golden_dir" "$tmp_dir"
    echo "writer_lease_decision_golden: lease + cascade scenarios byte-identical"
    ;;
  *)
    echo "usage: tests/writer_lease_decision_golden.sh [capture|verify] [golden-dir]" >&2
    exit 2
    ;;
esac
