;; ============================================================================
;; snapshot_warm_read_receipt.clj — interface investigation #1: warm read vs cold fold
;;   bb -cp out snapshot_warm_read_receipt.clj
;;
;; The MCP read path COLD-FOLDS the whole log per request (load-state -> fold(read-log));
;; the server's warm :query serves off the maintained in-memory store. Measures the
;; per-read tax: warm :query vs cold fold+query, and confirms the capability handshake
;; (database-query to a down/old server -> nil -> cold fallback).
;;
;; SAFE: synthetic /tmp log + in-process server (boot-flat!, handle{}); no socket, no
...
(println (format "WARM :query  (server warm store, no fold): median %.1f ms  [%.1f..%.1f]" (med warm) (apply min warm) (apply max warm)))
...
(println "fallback handshake (database-query to DEAD port 19998 -> nil -> cold path):" (nil? (fram.rt/database-query 19998 Q)))

(println "\n=== VERDICT ===")
(if (and (< (med warm) (med cold)) (pos? wq) (nil? (fram.rt/database-query 19998 Q)))
  (println "PASS — warm read path serves the same result far cheaper than the per-read cold fold,"
           "and falls back cleanly when the server is down/old. Rep-stable (Datalog over (l,p,r)).")
