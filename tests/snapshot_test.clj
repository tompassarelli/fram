;; snapshot_test.clj — thread 019f100f-7fff gate.
...
;; from the image). Run: bb -cp out tests/snapshot_test.clj
(require '[fram.store :as c] '[fram.schema :as s] '[clojure.string :as str])
(load-file "server.clj")
;; this test IS the snapshot-boot machinery's gate — keep the default-on
;; activation explicit in-process (thread 019f2190; see snapshot_boot_test.clj
...
(defn full-migrate-triples [log] (live-name-triples (migrate-flat->database log)))
...
(let [checks (atom [])
  ...
  (def snap (write-snapshot! @db LOG))
  ...
  (let [base (current-seq @db)]        ; past the snapshot's own @snapshot:* facts
    (append-lines! LOG
  ...
  (chk "reload(tail-fold): live == saved ground truth" (= (live-name-triples @db) full-after-tail))
  ...
  (chk "boot(incremental): live == ground truth"  (= (live-name-triples @db) full-after-tail))
  ...
  (let [tail-only (->> (str/split-lines (slurp LOG))
    ...
    (write-sidecar! LOG (-> (read-sidecar LOG)
                            (assoc :byte_offset 0
                                   :log_identity (log-identity-of LOG))
                            (assoc-in [:logs :coordination]
                                      {:byte_offset 0
                                       :identity (log-identity-of LOG)}))))
  (boot-flat! LOG)
  (chk "boot(post-compaction): full state from image+tail" (= (live-name-triples @db) full-after-tail))
  (let [st (:store @db)]
  ...
  (let [before (live-name-triples @db)
    ...
    (chk "regression: reverted log REFUSED — live state preserved" (= before (live-name-triples @db)))
