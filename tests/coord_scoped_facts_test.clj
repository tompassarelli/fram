...
;; subjects.  Run:
;;   bb -cp out tests/coord_scoped_facts_test.clj
;;
...
(with-redefs [coord-daemon/client-view-facts-from
...
(let [failures (remove second @checks)]
  ...
  (if (seq failures)
    (do (println "\ncoord-scoped-facts:" (count failures) "FAILED")
        (System/exit 1))
    (println "\ncoord-scoped-facts:" (count @checks) "/" (count @checks) "PASS")))
