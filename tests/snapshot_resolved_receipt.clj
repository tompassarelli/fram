;; ============================================================================
;; snapshot_resolved_receipt.clj — interface investigation #3: resolution multiplicity
...
;;
;; SAFE: fresh in-process snapshot reader on a /tmp scratch log; no socket, no port 7977,
...
(register-pred! @db "fram_contested" "multi" "literal")

(commit! @db "a" "@T" "title" :assert "Target" 0)            ; uncontested: 1 live
(commit! @db "a" "@T" "fram_contested" :assert "alice" 0)    ; contested: 2 distinct live values
(commit! @db "b" "@T" "fram_contested" :assert "bob"   0)
