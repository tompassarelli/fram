;; ============================================================================
;; commit_crdt_commute_order.clj — #36 GATE 3: do same-gap concurrent inserts CONVERGE
;; to a delivery-order-INDEPENDENT total order? (commute, demonstrated not asserted)
;;   bb -cp out tests/commit_crdt_commute_order.clj
;;
;; Gate 2 (commit_crdt_insert_receipt) shows two concurrent same-gap inserts BOTH LAND at
...
(defn name-of [e] (let [NAME (c/value-id st "name")] (some->> (c/by-lp st e NAME) first (c/fact-of st) :r (c/literal st))))
(defn forms-of [wrap]
  (->> (c/by-l st wrap)
...
(cond
  ...
  (do (println "CHARACTERIZED (honest boundary) — both same-gap inserts share the between-path; the ORDER is")
      ...
      (println "is separately DETECTED (commit_crdt_coupled_receipt). Gate 3 result banked as that boundary.")
      (System/exit 0))
