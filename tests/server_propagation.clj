;; ============================================================================
;; server_propagation.clj — DEBT 1 PROPAGATION-LATENCY RECEIPT
;;   bb -cp out server_propagation.clj
;;
...
(load-file "server.clj")   ; pulls in database + resolve; tail dispatch = nil (no args)
...
(let [[boot-ms _] (timed (boot-flat! tmp-log))]
  (println (format "booted isolated daemon: %d live facts in %.0f ms"
                   (count (c/current-facts (:store @db))) boot-ms)))
(defn all-node-names [st]
  (let [NAME (c/value-id st "name")]
    (when NAME (keep #(c/literal st (:r (c/fact-of st %))) (c/by-p st NAME)))))
(def modules (vec (into (sorted-set) (keep module-of-name (all-node-names (:store @db))))))
...
(let [tgt (first (filter #(re-matches #"@[^#]+#\d+" (str %)) (all-node-names (:store @db))))]
  (when tgt
