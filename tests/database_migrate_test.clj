;; database_migrate_test.clj — Stage 7 migration gate: the LIVE flat log migrates,
;; LOSSLESS, into the reified substrate and out to a v2 reified log that the
;; database can boot from. Round-trip proven:
;;
...
;;   FRAM_LOG=/path bb -cp out database_migrate_test.clj
...
(when (or (nil? log) (not (.exists (io/file log))))
  (println "database_migrate_test: skipped — set FRAM_LOG to a facts.log to run")
  (System/exit 0))

;; database.clj is a script (dump-log!/replay live in `user`); load it for them.
