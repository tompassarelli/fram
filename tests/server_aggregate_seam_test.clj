;; server_aggregate_seam_test.clj — CROSS-LANE SEAM: aggregates + comparison
;; predicates (fram.query/fram.datalog lane) exercised THROUGH the daemon's real
;; execute-query dispatch (server.clj lane), in-process — the composition no
;; single lane tested. Mirrors tests/pull_test.clj section (15): boot the daemon's
;; db/schema/cache globals without a socket and drive execute-query via resolve.
;; Proves:
...
;;   bb -cp out tests/server_aggregate_seam_test.clj
(require '[fram.store :as c] '[fram.schema :as s]
         '[clojure.java.io :as io])
(load-file "database.clj")          ; new-database / register-pred! / commit! land in `user`
...
(let [loaded? (try (load-file "server.clj") true
                   (catch Throwable t
                     (println "FATAL: server.clj not loadable standalone —"
                              (.getMessage t))
                     false))]
  (when-not loaded? (System/exit 1)))

;; Daemon globals resolve at runtime (server_daemon loads after this file's analysis).
(def db-var     (resolve 'db))
...
(def db (new-database dlog))
(register-pred! db "depends_on" "multi" "ref")
(register-pred! db "cost" "single" "literal")

;; --- graph: 5 subjects, depends_on edges, numeric cost ----------------------
;; edges: @s1->@s2 @s1->@s3 @s2->@s3 @s4->@s5   (out-degree: @s1=2 @s2=1 @s4=1)
(commit! db "u" "@s1" "depends_on" :link "@s2" nil)
(commit! db "u" "@s1" "depends_on" :link "@s3" nil)
(commit! db "u" "@s2" "depends_on" :link "@s3" nil)
(commit! db "u" "@s4" "depends_on" :link "@s5" nil)
(commit! db "u" "@s1" "cost" :assert "10"  nil)
(commit! db "u" "@s2" "cost" :assert "50"  nil)
(commit! db "u" "@s3" "cost" :assert "150" nil)
...
(commit! db "u" "@s3" "cost" :assert "999" nil)
