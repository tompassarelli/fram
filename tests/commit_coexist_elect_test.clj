;; commit_coexist_elect_test.clj — the Keystone (move-B) gate: cardinality-as-a-fact is
...
(let [log "/tmp/store-coexist-elect-test.log"
  ...
  (chk "seed: EVERY kernel single-valued pred is now declared single via a cardinality FACT"
       (every? #(= "single" (s/cardinality (store db) %)) ck/single-valued))
  (chk "coexist default: an UNDECLARED predicate is multi (no cardinality fact)"
       (= "multi" (s/cardinality (store db) "zzz_coexist")))
  ...
  (let [seed  (commit! db "w0" "T2" "status" :assert "open" 0)
        base  (:ok seed)
        good  (commit! db "w1" "T2" "status" :assert "wip"  base)   ; base up-to-date -> wins, bumps base
        stale (commit! db "w2" "T2" "status" :assert "done" base)   ; SAME (now-stale) base -> reject
    ...
    (chk "declared-single: the live value is the winner's, not the stale write's"
         (= "wip" (c/literal (store db) (:r (c/fact-of (store db) (first live)))))))

  ;; ---- (c) multi-valued just APPENDS ----
  (let [a   (commit! db "w" "T3" "tag" :link "X" 0)
        b   (commit! db "w" "T3" "tag" :link "Y" 0)
        dup (commit! db "w" "T3" "tag" :link "X" 0)
        live (live-of "T3" "tag")]
