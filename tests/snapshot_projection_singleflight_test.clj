;; snapshot_projection_singleflight_test.clj — the snapshot reader's per-version Datalog
;; projection cache (snapshot_daemon/whole-corpus-projection + projection-cache +
...
;;
;; Loads server.clj in-process (its entry point is *command-line-args*-guarded,
;; so no server starts) and drives the real functions.
;;   bb -cp out tests/snapshot_projection_singleflight_test.clj
...
(reset! server/projection-cache nil)
...
(with-redefs [q/project! (let [orig q/project!]
  ...
  (let [p1 (server/whole-corpus-projection facts-a)
    ...
    (chk "cache holds that facts set" (identical? facts-a (:facts @server/projection-cache)))
    ...
    (let [n 32
          ...
          threads (mapv (fn [_]
                          (Thread.
                           (fn []
                             (.await barrier)
                             (swap! results conj (server/whole-corpus-projection facts-a)))))
                        (range n))]
    ...
    (reset! build-count 0)
    (let [pa (server/whole-corpus-projection facts-a)   ; still cached from (2)
          pb (server/whole-corpus-projection facts-b)]  ; miss → one rebuild
      ...
      (chk "cache advances to the new facts set (single retained entry)"
           (identical? facts-b (:facts @server/projection-cache))))))

;; --- (4) the cached projection actually answers the production page query -----
(let [proj (server/whole-corpus-projection facts-a)
...
(let [snapshot {:facts facts-a :idx (rotations/build (map (fn [f] [(:l f) (:p f) (:r f)]) facts-a))}
      ...
      pf (server/snapshot-projection snapshot plain)
      pi (server/snapshot-projection snapshot with-id)]
  ...
  (chk "the fact-id route reuses the single-flight cache"
       (identical? pi (server/snapshot-projection snapshot with-id)))
  ...
  (chk "rotations projection answers the plain query exactly as the whole corpus"
       (= (set (:ok (q/run-projected pf plain)))
          (set (:ok (q/run-projected (server/whole-corpus-projection facts-a) plain))))))
