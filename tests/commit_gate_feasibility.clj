;; ============================================================================
;; commit_gate_feasibility.clj — RED-vs-YELLOW: decompose the scoped-resolve floor
;;   bb -cp out commit_gate_feasibility.clj
;;
...
(load-file "server.clj")   ; loads resolve (ns resolve) + database; tail dispatch no-ops
...
(let [[bt _] (timed (boot-flat! tmp))]
  (println (format "booted: %d live facts in %.0f ms" (count (c/current-facts (:store @db))) bt)))
(def st (:store @db))
...
(println (format "name-facts=%d  modules(K)=%d  e.g. %s\n" (count name-cids) (count mods) (vec (take 6 mods))))
