...
;; Run: bb -cp out tests/server_admission_deadline_test.clj
...
(def root
  (.getCanonicalPath
   (io/file
    (or (System/getenv "FRAM_SERVER_TEST_ROOT")
        (System/getProperty "user.dir")))))
(def daemon-file
  (or (System/getenv "FRAM_SERVER_DAEMON")
      (str root "/server.clj")))
(def checks (atom []))
...
(def watchdog
  (future
    (Thread/sleep 60000)
    (binding [*out* *err*]
      (println "database-admission-deadline: hard timeout"))
    (System/exit 124)))
...
(let [port (free-port)
      ...
      env (assoc (into {} (System/getenv))
                 ...
                 "FRAM_SERVER_CONNECTION_WORKERS" "2"
                 "FRAM_SERVER_CONNECTION_QUEUE" "1"
...
(let [failures (remove second @checks)]
  ...
  (if (seq failures)
    (do
      (println "\nserver-admission-deadline:" (count failures) "FAILED")
      (System/exit 1))
    (println "\nserver-admission-deadline:"
