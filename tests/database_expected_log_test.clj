...
;; expecting log B. The expected-log check and mutation share the database
...
;;
;; Run: bb -cp out tests/database_expected_log_test.clj
...
(def watchdog
  (future
    (Thread/sleep 60000)
    (binding [*out* *err*]
      (println "database-expected-log: hard timeout after 60s"))
    (System/exit 124)))
...
(let [port (free-port)
      ...
      env-b {"FRAM_SERVER_PORT" (str port)
  ...
  (try
    ...
    (let [before-a (slurp log-a)
      ...
      (check! "mismatched warm reads do not expose log A"
              (and (nil? (rt/database-query-for-log
                          port (.getPath log-b)
                          {:find "x" :rules []}))
                   (empty? (rt/database-live-facts port (.getPath log-b)))))
    ...
    (let [version (rt/database-version-for-log port (.getPath alias-a))
          status (rt/database-status-for-log port (.getPath alias-a))
          asserted (rt/database-assert-for-log
                    port (.getPath alias-a) "@right" "note" "landed" version)
          live (rt/database-live-facts port (.getPath alias-a))
          subscribed (client port (for-log (.getPath alias-a)
                                           {:op :subscribe}))
          retracted (rt/database-retract-for-log
                     port (.getPath alias-a) "@right" "note" "landed"
                     (rt/database-version-for-log port (.getPath alias-a)))]
    ...
    (let [before-a (slurp log-a)
          ...
          _ (java.nio.file.Files/createSymbolicLink
             ...
             (make-array java.nio.file.attribute.FileAttribute 0))
          alias-version (rt/database-version-for-log port (.getPath alias-a))
          alias-write (rt/database-assert-for-log
                       port (.getPath alias-a)
                       "@retargeted-alias" "note" "never" 0)
          b-write (rt/database-assert-for-log
                   port (.getPath log-b)
          ...
          a-version (rt/database-version-for-log port (.getPath log-a))
          a-write (rt/database-assert-for-log
                   port (.getPath log-a)
    ...
    (let [seen (atom nil)
      ...
      (try
        (let [result (rt/database-assert-for-log
                      port (.getPath log-b)
    ...
    (let [strict-port (free-port)
      ...
      (try
        ...
        (let [raw-watch-error
              (try
                (rt/database-watch strict-port)
                nil
                (catch Throwable t (.getMessage t)))
              wrong-watch-error
              (try
                (rt/database-watch-for-log strict-port (.getPath strict-b))
                nil
                (catch Throwable t (.getMessage t)))]
        ...
        ;; Regression for database-watch-request inheriting database-socket's 2s request
        ...
        (let [watch
              (proc/process
               ...
               "bb" "-cp" "out" "-e"
               "(require '[fram.rt :as rt]) (rt/database-watch-for-log (Integer/parseInt (System/getenv \"WATCH_PORT\")) (System/getenv \"WATCH_LOG\"))")
              _ (reset! watch-process watch)
              _ (Thread/sleep 2600)
              alive-after-idle? (.isAlive ^Process (:proc watch))
              watch-version
              (rt/database-version-for-log strict-port (.getPath strict-a))
              watch-write
