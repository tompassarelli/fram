#!/usr/bin/env bash
# Original-first differential oracle for database.clj Cut A, the pure reader layer.
#
...
for case_name in "${cases[@]}"; do
  set +e
  bb -cp out tests/snapshot_reader_golden.clj "$case_name" \
...
case "$mode" in
  capture)
    ...
    echo "snapshot_reader_golden: captured ${#cases[@]} original-Clojure cases -> $golden_dir"
    ;;
  verify)
    for case_name in "${cases[@]}"; do
    ...
    echo "snapshot_reader_golden: ALL ${#cases[@]} cases byte-identical"
    ;;
  *)
    echo "usage: tests/snapshot_reader_golden.sh [capture|verify] [golden-dir]" >&2
    exit 2
    ;;
esac
