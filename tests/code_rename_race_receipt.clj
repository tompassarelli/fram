;; ============================================================================
;; code_rename_race_receipt.clj — #(a) rename under concurrency: the snapshot-window race.
;;   bb -cp out code_rename_race_receipt.clj   (needs BEAGLE_HOME for the cold render)
;;
...
(defn cold-render-line [log marker]
  (let [out (str "/tmp/rename-race-" (System/nanoTime) ".bclj")
        module (resolve-module (:store (migrate-flat->database log)) "schema")]
