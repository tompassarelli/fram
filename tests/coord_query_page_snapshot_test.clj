...
;; Run: bb -cp out tests/coord_query_page_snapshot_test.clj
...
(let [failures (remove second @checks)]
  ...
  (if (seq failures)
    (do (println "\ncoord-query-page-snapshot:" (count failures) "FAILED")
        (System/exit 1))
    (println "\ncoord-query-page-snapshot:" (count @checks) "/" (count @checks) "PASS")))
