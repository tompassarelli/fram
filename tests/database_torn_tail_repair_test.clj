...
;; the in-memory store.
;;
;; Run: bb -cp out tests/database_torn_tail_repair_test.clj
...
(def coordination-prefix
  (str (flat-line 1 "@lease" "cardinality" "single")
       ...
       (flat-line 3 "@database-seed" "title" "keep-coordination")))
...
(try
  ...
  (let [database-before (bytes coordination)
    ...
    (try
      ...
      (check! "exclusive repair waits behind an existing shared append admission"
              (and (not (ready? port))
                   (bytes= database-before (bytes coordination))
    ...
    (check! "all complete pre-crash facts remain queryable"
            (and (= #{"keep-coordination"}
                    (values-of port "@database-seed" "title"))
                 (= #{"keep-telemetry"}
                    (values-of port "@run-seed" "note"))))

    (let [database-retry (client port {:op :assert :te "@database-retry"
      ...
      (check! "retry appends succeed after physical repair"
              (and (:ok database-retry) (:ok telemetry-retry)
                   (terminal-lf? coordination) (terminal-lf? telemetry)
                   (= #{"coordination-retry"}
                      (values-of port "@database-retry" "note"))
    ...
    (let [database-before-refusal (bytes coordination)
      ...
      (check! "live mutation refuses either split log at a non-LF boundary"
              (and (:error refused)
                   (bytes= database-before-refusal (bytes coordination))
    ...
    (stop-daemon!)
    (let [database-stable (bytes coordination)
          telemetry-stable (bytes telemetry)
          complete-database (pr-str {:tx 100 :op "assert" :l "@complete-database"
                                  :p "note" :r "preserve-complete-database"
                                  :frame "torn-tail-test"})
          complete-telemetry
          (pr-str {:tx 101 :op "assert" :l "@complete-telemetry"
                   :p "kind" :r "run" :frame "torn-tail-test"})
          _ (write-raw! coordination complete-database true)
          _ (write-raw! telemetry complete-telemetry true)
          {:keys [port]} (start-daemon!)]
      (await-daemon! port)
      (check! "complete unterminated coordination record is preserved"
              (and (= (str (String. database-stable "UTF-8") complete-database "\n")
                      (slurp coordination))
                   (= #{"preserve-complete-database"}
                      (values-of port "@complete-database" "note"))))
