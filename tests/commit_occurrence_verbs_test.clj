;; commit_occurrence_verbs_test.clj — the move-C gate: per-(s,p) cardinality-typed write verbs
...
;; (the wire verbs append!/put!/swap! in north.database are thin shells over these
...
(let [log "/tmp/store-occ-verbs-test.log"
      db (new-database log)
      _ (register-pred! db "status" "single" "literal")   ; declared-single via a cardinality FACT
      _ (register-pred! db "tag" "multi" "ref")            ; declared multi
      checks (atom [])
      chk (fn [nm ok] (swap! checks conj [nm ok]))
      live-of (fn [te-name pred]
                (vec (live-cids-lp db (s/resolve-name (store db) te-name)
  ...
  (let [fs   (mapv (fn [v] (future (commit! db (str "w" v) "M1" "tag" :link v nil)))   ; nil base
                   ["X" "Y"])
        rs   (mapv deref fs)
        live (live-of "M1" "tag")]
  ...
  (let [r1   (commit! db "w1" "P1" "status" :assert "open" nil)   ; nil base
        r2   (commit! db "w2" "P1" "status" :assert "done" nil)   ; nil base — supersedes
  ...
  (let [seed  (commit! db "w0" "P2" "status" :assert "a" 0)
        base  (:ok seed)
        good  (commit! db "w1" "P2" "status" :assert "b" base)   ; up-to-date base wins, bumps base
        stale (commit! db "w2" "P2" "status" :assert "c" base)   ; SAME (now-stale) base -> reject
  ...
  (let [seed  (commit! db "w0" "P3" "status" :assert "a" 0)
        _     (commit! db "w1" "P3" "status" :assert "b" (:ok seed))   ; value MOVES; base advances
        nb    (commit! db "w2" "P3" "status" :assert "c" nil)          ; NO base, against a moved single pred
    ...
    (chk "base-optional: swap! (base) still rejects a stale base on the same pred"
         (= :conflict (:reject (commit! db "w3" "P3" "status" :assert "d" (:ok seed))))))
