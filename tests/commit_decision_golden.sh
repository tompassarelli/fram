#!/usr/bin/env bash
# Original-first one-case OCC decision oracle for database.clj M5 Cut B.
#
...
set +e
bb -cp out tests/commit_decision_golden.clj \
...
case "$mode" in
  capture)
    ...
    echo "commit_decision_golden: captured original conflict scenario -> $golden_dir"
    ;;
  verify)
    ...
    echo "commit_decision_golden: original conflict scenario byte-identical"
    ;;
  *)
    echo "usage: tests/commit_decision_golden.sh [capture|verify] [golden-dir]" >&2
    exit 2
    ;;
esac
