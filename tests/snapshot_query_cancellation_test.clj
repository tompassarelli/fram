;; Forced-failure proof for snapshot reader query isolation and cancellation.
;; Run: bb -cp out tests/snapshot_query_cancellation_test.clj
...
(def watchdog
  (future
    (Thread/sleep 60000)
    (binding [*out* *err*] (println "database-query-cancellation: hard timeout"))
    (System/exit 124)))
...
(let [failures (remove second @checks)]
  ...
  (if (seq failures)
    (do (println "\nsnapshot-query-cancellation:" (count failures) "FAILED")
        (System/exit 1))
    (println "\nsnapshot-query-cancellation:" (count @checks) "/" (count @checks) "PASS")))
