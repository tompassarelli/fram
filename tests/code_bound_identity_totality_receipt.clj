;; ============================================================================
;; code_bound_identity_totality_receipt.clj — #(a) identity-FIRST totality.
;;   bb -cp out code_bound_identity_totality_receipt.clj
;;
...
(defn ref-leaves []
  (let [st (:store @db) REFp (c/value-id st "refers_to")]
    (->> (c/by-p st REFp) (map #(c/fact-of st %))
         (filter #(integer? (:r %))) (map :l) set)))
(defn bound-by-leaf []
  (let [st (:store @db) BND (c/value-id st "bound_to")]
