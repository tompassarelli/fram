;; database_schema_read_test.clj — F4: schema-writable facts are READ-VISIBLE.
...
;; Run: bb -cp out tests/database_schema_read_test.clj
...
;; the daemon's :facts-op view (what database serves the CLI): client read view = domain +
;; log-sourced schema facts, in fold-emission order — exactly the :facts op's computation.
(defn daemon-triples [] (mapv (fn [c] [(:l c) (:p c) (:r c)]) (fold/refold-order (client-view-facts @db))))
