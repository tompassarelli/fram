;; database_cardinality_fact_test.clj — the finding #23 daemon-seam gate.
...
;; Run: bb -cp out tests/database_cardinality_fact_test.clj
...
(let [checks (atom [])
  ...
  (let [db (migrate-flat->database LOG) st (:store db) t1 (s/resolve-name st "@T1")]
    ...
    (chk "CLI ≡ daemon: domain triples set-equal" (= (cli-domain LOG) (live-name-triples db))))
  ...
  (chk "B/reload: live == cold CLI fold (daemon≡CLI over the tail)"
       (= (cli-domain LOG) (live-name-triples @db)))
