;; Deterministic regression for the server's global-monitor convoy.
;; Run: bb -cp out tests/server_lock_convoy_test.clj
...
(def watchdog
  (future
    (Thread/sleep 60000)
    (binding [*out* *err*]
      (println "database-lock-convoy: hard timeout"))
    (System/exit 124)))
...
(let [failures (remove second @checks)]
  ...
  (if (seq failures)
    (do (println "\nserver-lock-convoy:" (count failures) "FAILED")
        (System/exit 1))
    (println "\nserver-lock-convoy:"
