;; ============================================================================
;; code_mcp_insert_after_receipt.clj — §0.2 blocker closed: a FLUENT AGENT can now
;; reach insert-anywhere-commute.
;;   bb -cp out code_mcp_insert_after_receipt.clj
;;
...
;; one-shot EDN request to a server (same wire the CLI uses).
(defn- database-rt [req]
...
(def daemon
  (proc/process ["clojure" "-M" "server.clj" "serve-flat" (str port) tmp-log]
                {:dir here :out :inherit :err :inherit}))
;; wait until it answers :status (up to ~180s — JVM start + 9MB log migrate is slow).
(def up?
  (loop [n 0]
    (if (>= n 360) false
      (let [st (try (database-rt {:op :status}) (catch Exception _ nil))]
...
;; code_crdt_insert_receipt does, so we get resolve/ord-parse for free.
...
(def ro (migrate-flat->database tmp-log))
(def rst (:store ro))
...
(def tool-store (store/new-term-store "database-mcp-insert-after"))
(def cat (tl/catalog []))
...
(def ro2 (migrate-flat->database tmp-log))
(def rst2 (:store ro2))
