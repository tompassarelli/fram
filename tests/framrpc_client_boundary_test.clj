...
(def child-form
  (str
   ...
   "(case mode "
   "\"edn\" (prn (rt/database-request-for-log port log {:op :version})) "
   "\"edn-size\" (println (count (:payload "
   "(rt/database-request-for-log port log {:op :version})))) "
   "\"live-size\" (let [facts (rt/database-live-facts port log)] "
   "(println (if-let [fact (first facts)] (count (:r fact)) 0))) "
   "\"live-count\" (println (count (rt/database-live-facts port log))) "
   "\"watch\" (rt/database-watch-for-log port log) "
   ...
   "(.setSoTimeout socket "
   "(Integer/parseInt (System/getenv \"FRAM_SERVER_READ_TIMEOUT_MS\"))) "
   "((deref (var fram.rt/database-tls-handshake!)) socket)) "
   "\"tls-init-close\" "
   "(let [captured (atom nil) "
   "connect! (deref (var fram.rt/database-socket)) "
   "context (javax.net.ssl.SSLContext/getDefault)] "
   "(try "
   "(with-redefs-fn "
   "{(var fram.rt/client-ssl-context) (fn [& _] context) "
   "(var fram.rt/database-tls-handshake!) "
   "(fn [socket] "
   ...
   "run! (deref (var fram.rt/run-with-database-watchdog!))] "
    ...
    {:continue true
     ...
     (merge
      {"FRAM_TEST_PORT" (str port)
       ...
       "FRAM_TEST_MODE" mode
       "FRAM_SERVER_CONNECT_TIMEOUT_MS" "500"
...
(let [payload-size (* 512 1024)
      ...
      (run-peer
       "edn-size"
       {"FRAM_SERVER_READ_TIMEOUT_MS" "3000"
        "FRAM_SERVER_MAX_RESPONSE_BYTES" "1048576"}
       (fn [socket]
         (request-line! socket)
...
(let [payload-size (* 9 1024 1024)
      ...
      (run-peer
       "live-size"
       {"FRAM_SERVER_FACTS_TIMEOUT_MS" "10000"
        "FRAM_SERVER_READ_TIMEOUT_MS" "invalid"}
       (fn [socket]
         (request-line! socket)
...
(let [{:keys [result elapsed-ms]}
      (run-peer
       "live-count"
       {"FRAM_SERVER_READ_TIMEOUT_MS" "5000"
        "FRAM_SERVER_FACTS_TIMEOUT_MS" "150"}
       (fn [socket]
         (request-line! socket)
         (let [output (.getOutputStream socket)]
           (try
...
(let [result
      (child
       1
       "live-count"
       {"FRAM_SERVER_FACTS_TIMEOUT_MS" "0"})]
  (check! "facts deadline configuration is range-validated"
          (failed-with?
           result
           "FRAM_SERVER_FACTS_TIMEOUT_MS must be an integer from 1 through 999999")))
...
(let [{:keys [result]}
      (run-peer
       "edn"
       {"FRAM_SERVER_MAX_RESPONSE_BYTES" "64"}
       (fn [socket]
         (request-line! socket)
...
(let [{:keys [result elapsed-ms]}
      (run-peer
       "tls"
       {"FRAM_SERVER_HANDSHAKE_TIMEOUT_MS" "150"
        "FRAM_SERVER_READ_TIMEOUT_MS" "25"}
       (fn [socket]
