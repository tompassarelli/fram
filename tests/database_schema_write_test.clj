;; database_schema_write_test.clj — F3: the VALIDATED schema-write gate.
...
;; store booted via boot-flat!, NEVER the live 7977 database):
...
;; Run: bb -cp out tests/database_schema_write_test.clj
...
(let [checks (atom [])
  ...
  (let [res (do-assert "@rel" "cardinality" "single" nil)
        st  (:store @db) p1 (s/resolve-name st "@P1")]
