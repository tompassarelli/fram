...
FRAM_GROUP_GOLDEN_DIR="$tmp_dir/scenario" \
  bb -cp out tests/commit_group_decision_golden.clj \
...
case "$mode" in
  capture)
    ...
    echo "commit_group_decision_golden: captured original batching + queue scenario -> $golden_dir"
    ;;
  verify)
    ...
    echo "commit_group_decision_golden: batching + queue scenario byte-identical"
    ;;
  *)
    echo "usage: tests/commit_group_decision_golden.sh [capture|verify] [golden-dir]" >&2
    exit 2
    ;;
esac
