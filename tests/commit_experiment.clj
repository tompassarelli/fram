;; ============================================================================
;; commit_experiment.clj — #11 R1 git arm, SAME-ARTIFACT (beagle-text-vs-beagle-graph).
;; HARDENED: the git arm now edits the RENDERED TEXT of the REAL `schema` module — the SAME
;; module the Fram arm edits via the fact graph (database_fram_r1.clj). Previously the git
;; arm used a SYNTHETIC disjoint-functions stand-in; the commit-sequencer COUNTS are content-
...
;;   bb -cp out commit_experiment.clj
;; ============================================================================
...
;; the SAME functions the Fram R1 arm edits (database_fram_r1.clj).
...
(defn git-r1 [k]
  (let [dir (str (System/getProperty "java.io.tmpdir") "/coordexp-sa-" k "-" (System/nanoTime))
    ...
    (let [t0 (System/nanoTime)
          ...
          landed (count (filter (fn [f] (str/includes? merged (str "database-edit-" f))) fns))
