...
;; survive process replacement in writer-authority.log, never telemetry.log.
;;
;; Run: bb -cp out tests/writer_lease_flat_durability_test.clj
    ...
    (let [port-2 (start-daemon!)
      ...
      (let [port-3 (start-daemon!)
        ...
        (let [port-4 (start-daemon!)
              ...
              database-write (client port-4 {:op :assert :te "@seed" :p "note"
                                          :r "still-normal"})
          ...
          (check! "ordinary split-corpus write routing remains unchanged"
                  (and (:ok database-write) (:ok telemetry-write)
