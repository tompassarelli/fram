;; Exact coordinator :show must remain O(subject), including immediately after
;; an unrelated write changes the global version.  Run:
;;   bb -cp out tests/coord_show_indexed_test.clj
...
(with-redefs [coord-daemon/facts-wire-snapshot
              (fn [& _]
                (throw (ex-info "whole wire projection selected" {})))
              coord-daemon/client-view-facts-from
              (fn [& _]
...
(let [failures (remove second @checks)]
  ...
  (if (seq failures)
    (do
      (println "\ncoord-show-indexed:" (count failures) "FAILED")
      (System/exit 1))
    (println "\ncoord-show-indexed:"
