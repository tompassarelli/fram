;; commit_batch_test.clj — in-process receipt for commit-batch!, the atomic
...
;;
;; Run: bb -cp out tests/commit_batch_test.clj
(load-file "database.clj")

(def checks (atom []))
(defn ck! [l v] (swap! checks conj [l (boolean v)]))
(defn fresh [] (new-database (str "/tmp/cb-" (System/nanoTime) "-" (rand-int 1e6) ".log")))

;; live values on (te,pred), resolved back to strings
(defn vals-of [db te p]
  (let [st (:store db)
    ...
    (if (and pid teid)
      (set (map #(let [r (:r (fram.store/fact-of st %))]
                ...
                (#'database/live-cids-lp db teid pid)))
      #{})))

;; (1) happy path — three multi facts land as one unit
(let [db (fresh)
      r (commit-batch! db "database" "@msg:1"
  ...
  (ck! "all facts live" (and (= #{"@a"} (vals-of db "@msg:1" "from"))
                             (= #{"hi"} (vals-of db "@msg:1" "subject"))
                             (= #{"@b"} (vals-of db "@msg:1" "to")))))
...
(let [db (fresh)
      v0 (current-seq db)
      r (commit-batch! db "database" "@msg:2"
  ...
  (ck! "zero facts landed" (and (empty? (vals-of db "@msg:2" "subject"))
                                (empty? (vals-of db "@msg:2" "depends_on"))))
  (ck! "version unmoved after reject" (= v0 (current-seq db))))
...
(let [db (fresh)
      _ (commit! db "database" "@msg:3" "body" :assert "first" nil)  ; body is single-valued
      r (commit-batch! db "database" "@msg:3"
                       [{:pred "from" :kind :assert :r "@a"}
  ...
  (ck! "no partial: from absent, body unchanged"
       (and (empty? (vals-of db "@msg:3" "from"))
            (= #{"first"} (vals-of db "@msg:3" "body")))))

;; (4) idempotency — re-publishing identical multi facts writes nothing (once, not twice)
(let [db (fresh)
      _ (commit-batch! db "database" "@msg:4" [{:pred "from" :kind :assert :r "@x"}])
      r (commit-batch! db "database" "@msg:4" [{:pred "from" :kind :assert :r "@x"}])]
  (ck! "re-batch writes nothing" (empty? (:written r)))
  (ck! "re-batch marks idempotent" (= ["from"] (:idempotent r)))
  (ck! "single live value, no dup" (= #{"@x"} (vals-of db "@msg:4" "from"))))
...
(let [db (new-database (str "/tmp/cb-durable-" (System/nanoTime) ".log"))
      before (:next-seq @(:store db))
      _ (commit-batch! db "database" "@msg:5"
                       [{:pred "from" :kind :assert :r "@a"}
...
(let [db (fresh)
      _ (commit-batch! db "database" "@msg:6" [{:pred "from" :kind :assert :r "@a"}])
      r (commit-batch! db "database" "@msg:6"
  ...
  (ck! "both values live" (and (= #{"@a"} (vals-of db "@msg:6" "from"))
                               (= #{"new"} (vals-of db "@msg:6" "subject")))))
