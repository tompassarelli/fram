...
;; Run: bb -cp out tests/snapshot_query_isolation_test.clj
...
(let [failures (remove second @checks)]
  ...
  (if (seq failures)
    (do (println "\nsnapshot-query-snapshot-isolation:" (count failures) "FAILED")
        (System/exit 1))
    (println "\nsnapshot-query-snapshot-isolation:" (count @checks) "/" (count @checks) "PASS")))
