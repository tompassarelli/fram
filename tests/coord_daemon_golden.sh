#!/usr/bin/env bash
# coord_daemon golden gate — eleven original-Clojure behavior comparisons.
#
# Capture precedes any Beagle port:
#   tests/coord_daemon_golden.sh capture tests/goldens/coord_daemon
# Verify later implementations byte-for-byte:
#   tests/coord_daemon_golden.sh verify tests/goldens/coord_daemon
#
...
# that forbid TCP listen(2) can set FRAM_COORD_GOLDEN_TRANSPORT=memory; that mode
# passes the same bytes through coord-daemon/serve-conn using a test-only Socket
...
MODE="${1:?usage: coord_daemon_golden.sh capture|verify <dir>}"
GOLD="${2:?usage: coord_daemon_golden.sh capture|verify <dir>}"
DAEMON="${FRAM_COORD_DAEMON:-$HERE/coord_daemon.clj}"
WORK="${TMPDIR:-/tmp}/coord-daemon-golden-run"
TRANSPORT="${FRAM_COORD_GOLDEN_TRANSPORT:-port}"

case "$MODE" in
  capture|verify) ;;
  *) echo "coord_daemon_golden: mode must be capture or verify" >&2; exit 2 ;;
esac
case "$TRANSPORT" in
  port|memory) ;;
  *) echo "coord_daemon_golden: transport must be port or memory" >&2; exit 2 ;;
esac

if command -v flock >/dev/null 2>&1; then
  exec 9>"$WORK.lock"
  flock -w 1800 9 || {
    echo "coord_daemon_golden: another run holds $WORK.lock" >&2
    exit 2
  }
fi
...
(def case-dir (io/file "/tmp/coord-daemon-golden-run/cases" case-name))
(.mkdirs case-dir)
...
(case case-name
  ...
  "migrate-flat"
  (do (seed!)
      (let [co0 (migrate-flat->co flat)]
        (prn {:version (current-seq co0)
              :triples (domain-triples co0)})))

  "boot-existing"
  (do (seed!)
      (golden-boot!)
      (prn {:version (current-seq @co)
            :facts (hybrid-fact-count)
            :mode (:mode @last-boot)
            :reason (:reason @last-boot)
            :title (s/lookup (:store @co)
                             (s/resolve-name (:store @co) "@A")
                             "title")}))
  ...
  (throw (ex-info "unknown coord_daemon golden case" {:case case-name})))
EOF
...
for case_name in "${CASES[@]}"; do
  i=$((i + 1))
  echo "coord_daemon_golden: RUN $i/${#CASES[@]} $case_name [$TRANSPORT]"
  run_case "$case_name"
done
...
for case_name in "${CASES[@]}"; do
  if [ "$(tr -d '\n' <"$WORK/$case_name.rc")" != 0 ]; then
    echo "coord_daemon_golden: case failed: $case_name (rc $(cat "$WORK/$case_name.rc"))" >&2
...
if [ "$MODE" = capture ]; then
  ...
  echo "coord_daemon_golden: captured ${#CASES[@]} cases -> $GOLD"
  exit 0
fi
...
if [ "$fail" = 0 ]; then
  echo "coord_daemon_golden: ALL ${#CASES[@]} cases byte-identical (zero masks)"
fi
exit "$fail"
