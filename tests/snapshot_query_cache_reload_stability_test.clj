...
;; Run: bb -cp out tests/snapshot_query_cache_reload_stability_test.clj
...
(def watchdog
  (future
    (Thread/sleep 480000)
    (binding [*out* *err*]
      (println "database-query-cache-reload-stability: hard timeout after 480s"))
    (System/exit 124)))
...
(let [port (free-port)
  ...
  (try
    ...
    ;; External tail + two simultaneous reloaders + a normal snapshot reader append.
...
(let [failures (remove second @checks)]
  ...
  (if (seq failures)
    (do (println "\nsnapshot-query-cache-reload-stability:" (count failures) "FAILED")
        (System/exit 1))
    (println "\nsnapshot-query-cache-reload-stability:"
