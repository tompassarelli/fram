...
;;
;; Run: bb -cp out tests/writer_lease_external_freshness_test.clj
...
(spit coordination
      (str (flat-line 1 "@lease" "cardinality" "single")
           ...
           (flat-line 3 "@database-seed" "note" "coordination-seed")))
(spit telemetry
...
(try
  (let [port (start-daemon!)
        database-tx (+ 1000 (:version (client port {:op :version})))
        generation-before (generation port)
        _ (append-line! coordination database-tx "@database-external" "note" "database-external")
        lease-database (acquire! port "after-coordination-external")]
    ...
    (check! "next query absorbs the external coordination prefix"
            (= #{"database-external"} (values-of port "@database-external" "note")))
    ...
    (check! "lease survives the coordination-prefix reload"
            (held? port "after-coordination-external" lease-database))
    (check! "reload advances the unified version through the external tx"
            (>= (:version (client port {:op :version})) database-tx))

    (stop-daemon!)
    (let [port (start-daemon!)]
      (check! "restart retains both the external coordination fact and lease"
              (and (= #{"database-external"}
                      (values-of port "@database-external" "note"))
                   (held? port "after-coordination-external" lease-database)))
      ...
      (let [telemetry-tx (+ 1000 (:version (client port {:op :version})))
        ...
        (check! "both leases survive the telemetry-prefix reload"
                (and (held? port "after-coordination-external" lease-database)
