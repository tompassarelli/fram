...
(defn start-daemon [port log role token]
  (process/process
   ["bb" "-cp" "out:." "coord_daemon.clj" "serve-flat"
    (str port) (str log)]
   {:out :string
    :err :string
    :extra-env {"FRAM_COORD_ROLE" role
              ...
              (let [response
                ...
                (and (= :cutover-instance-mismatch (:code response))
                     (get-in
                      ...
                      [:writer-authority :write-authorized]))))
      (reset! standby (start-daemon standby-port log "standby" token))
      (let [standby-status
            (eventually
        ...
        (let [written (atom [])
          ...
          (let [bad-marker
                (assoc-in marker-1 [:logs 0 :boundary-sha]
                ...
                (request!
                 ...
                 (fenced
                  ...
                  {:op :cutover-promote
                   ...
                   :marker bad-marker}))
                _ (check!
                   "marker mismatch refuses promotion and releases authority"
                   (and (= :cutover-marker-mismatch
                           (:code bad-promotion))
                        (false?
                         (get-in
                          (cutover-status standby-port log token)
                          [:writer-authority :write-authorized]))))
                promote-run
                (cutover-cli!
                 "promote"
