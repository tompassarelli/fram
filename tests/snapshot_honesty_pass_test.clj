;; ============================================================================
;; snapshot_honesty_pass_test.clj — proves the read-side honesty pass is a
...
;; END-TO-END half (same RESOLVED outputs) is the existing suite: snapshot_code_flip
;; KEYSTONE-A (render(log) == render(text) BYTE-IDENTICAL — exercises
;; pred-val/kind-of/sym-val/refers-target) + snapshot_edit_min_scoped_correct
;; (refers_to symdiff 0). Run all three; green = behavior preserved.
;;   bb -cp out snapshot_honesty_pass_test.clj
;; ============================================================================
(binding [*command-line-args* []] (load-file "server.clj"))  ; loads ns `resolve`
