...
(spit database (str (pr-str {:tx 1 :op "assert" :l "@run-e2e" :p "kind" :r "run" :frame "test"}) "\n"
...
(try
  (await-ready child port #(not (neg? (rt/database-version %))))
  (let [v (rt/database-version port)
        telemetry-resp (rt/database-assert port "@run-e2e" "note" "routed" v)
        v2 (rt/database-version port)
        coordination-resp (rt/database-assert port "@thread-e2e" "owner" "personal" v2)
        v3 (rt/database-version port)
        legacy-routing-resp (rt/database-assert port "@audit-e2e" "note" "legacy-policy" v3)
        warm (rt/database-live-facts port database)]
    ...
    (assert (str/includes? (slurp database) ":p \"owner\""))
