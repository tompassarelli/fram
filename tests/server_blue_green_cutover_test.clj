;; server_blue_green_cutover_test.clj — steady-state authority handoff contract.
...
(reset! server/active-queries 1)
(reset! server/active-reloads 0)
(reset! server/active-snapshot-builds 0)
(let [release (future
                (Thread/sleep 80)
                (reset! server/active-queries 0))
      started (System/nanoTime)
      drain (with-redefs [server/cutover-drain-timeout-ms 1000]
              (server/await-cutover-drain!))
...
(reset! server/active-queries 1)
(let [timeout-code
      (with-redefs [server/cutover-drain-timeout-ms 20]
        (try
          (server/await-cutover-drain!)
          nil
...
(let [dir (.toFile
      ...
      ;; `server.log` intentionally activates the production split pair;
