...
(let [dir (or (System/getenv "FRAM_GROUP_GOLDEN_DIR")
                 ...
                 (fn [ctx]
                   (swap! events conj
                          {:label label
                           :ctx ctx
                           :group-lock-held
                           (Thread/holdsLock coord/group-io-lock)})))
      path-label (fn [path]
                   (cond
                     (= path path-a) :a
                     (= path path-b) :b
  ...
  (with-redefs [rt/with-append-admission
    ...
    (let [initial-results (mapv deref [t-a1 t-b1 t-a2 t-barrier])
          ...
          deferred-return
          (binding [coord/*durable-tickets* deferred-tickets]
            (coord/enqueue-durable! path-a ["a3\n"] (callback :a3)))
          deferred-result (deref deferred-return)
          inline-result
          (binding [coord/*durable-tickets* nil]
            (coord/enqueue-durable! path-b ["b2\n"] (callback :b2)))]
      (prn
