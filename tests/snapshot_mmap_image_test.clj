;; snapshot_mmap_image_test.clj — FRAM_MMAP_IMAGE V1 (thread 019f82d9).
...
;; Run from the repo root: bb -cp out tests/snapshot_mmap_image_test.clj
...
(let [checks (atom [])
  ...
  (chk "flag-off: state == whole-migrate ground truth" (= (live-name-triples @db) truth-a))
  ...
  (chk "hybrid-fact-count(cold) == materialized store live-fact count"
       (= cold-count (count (c/current-facts (:store @db)))))
  (chk "materialize: cold-image dropped" (nil? @cold-image))
  (chk "materialize: state == whole-migrate ground truth" (= (live-name-triples @db) truth-b))
  (chk "materialize: snapshot-reconcile green" (:ok (snapshot-reconcile @db LOG)))
  (let [st (:store @db) a (s/resolve-name st "@A")]
  ...
  (chk "tail: state == whole-migrate ground truth" (= (live-name-triples @db) truth-d))
  (let [st (:store @db) a (s/resolve-name st "@A")]
  ...
  (chk "corrupt segment: state still correct via fallback" (= (live-name-triples @db) truth-e))
  ...
  (ensure-materialized!)
  (chk "edge: materialize state correct" (= (live-name-triples @db) truth-f))
