;; database_dropin_test.clj — Stage 7 drop-in gate: the reified daemon as a reversible
...
;; live file.
;;   FRAM_LOG=/path bb -cp out database_dropin_test.clj
...
(when (or (nil? live) (not (.exists (io/file live))))
  (println "database_dropin_test: skipped — set FRAM_LOG") (System/exit 0))
...
(def boot-equal (= (domain-triples (:store @db)) (flat-set flat)))
...
(def after-write-has (contains? (domain-triples (:store @db)) ["@dropin-subj" "title" "Drop-in test"]))
...
(def absorbed (contains? (domain-triples (:store @db)) ["@ext-subj" "title" "External import"]))
(def daemon-write-survived (contains? (domain-triples (:store @db)) ["@dropin-subj" "title" "Drop-in test"]))
...
;; FRESH). Also: database.clj must still be able to fold the post-write log (rollback).
...
(def version-fresh (= (current-seq @db) (:version (fold/fold (fram.rt/read-log flat)))))
...
(def fresh-with-torn-tail (= (current-seq @db) (:version (fold/fold (fram.rt/read-log flat)))))
