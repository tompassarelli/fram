...
;;
;; Run: bb -cp out tests/coord_appender_failure_test.clj
(load-file "coord.clj")
...
(let [dir (.toFile
      ...
      outcome
      (with-redefs-fn
        {#'coord/plan-group-batch (fn [_] (throw injected))}
        (fn []
          (let [result
                (future
                  (try
                    (coord/enqueue-durable! log ["never-written\n"] nil)
                    :unexpected-success
                    (catch Throwable t t)))]
...
(let [failures (remove second @checks)]
  ...
  (if (seq failures)
    (do
      (println "\ncoord-appender-failure:" (count failures) "FAILED")
      (System/exit 1))
    (println "\ncoord-appender-failure:"
