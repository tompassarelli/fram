...
;; graph-upstream SEED and database.clj carries no world persistence layer yet, so
...
;;
;; NORMATIVE PERSISTENCE SURFACE (database.clj, load-file'd into the caller's ns —
;; the same idiom tests/commit_occurrence_verbs_test.clj uses for commit!/register-pred!):
;;   (world-create!   db agent name version-id)            -> {:ok seq} | {:reject kw}
;;   (world-fork!     db agent new-name version-id)         -> {:ok seq} | {:reject kw}
;;   (world-head      db name)                              -> VersionId | nil  (DERIVED)
;;   (world-blob-put! db agent ^bytes raw)                  -> {:ok blob-id} | {:reject kw}
;;   (world-blob      db blob-id)                           -> ^bytes | nil
;;   (world-version   db version-id)                        -> Version record | nil
;;   (world-manifest  db version-id)                        -> resolved manifest vector
;;   (world-begin!    db agent name expected-head nonce)    -> {:ok candidate-id} | {:reject kw}
;;   (world-append!   db agent candidate-id op)             -> {:ok index} | {:reject kw}
;;   (world-seal!     db agent candidate-id)                -> {:ok version-id} | {:reject kw}
;;   (world-candidate db candidate-id)   -> {:ops [..] :sealed <version-id|nil>} | nil
;;   (world-lock!     db version-id build-spec)             -> {:ok lock-id :lock rec}
;;   (world-build!    db agent lock-id)                     -> {:ok receipt} | {:reject kw}
;;   (world-promote!  db agent name expected-head candidate-id receipt)
...
(require '[clojure.edn :as edn]
         ...
         '[fram.rt :as rt])
(load-file "database.clj")   ; new-database / commit! / replay / assert-flat-append-boundary!
...
(defn uv
  "The Var for a world persistence fn defined by database.clj, or a catchable absence."
  [nm]
  (or (resolve (symbol nm))
      (throw (ex-info (str nm " ABSENT — database.clj has no world persistence layer yet")
                      {:missing nm}))))
...
(defn copy-log
  ...
  (let [dst (str scratch "/" tag "-" (swap! copies inc) ".log")]
    ...
    dst))
(defn reopen
  "Cold restart, exactly the daemon's own idiom (server.clj:4530):
...
(when-not (:ok kernel)
  (println (str "  NOTE: " (:why kernel) "; database.clj world verbs are also absent"
...
(def seam
  (delay
    (let [db (new-database seam-log)]
      (register-pred! db "status" "single" "literal")
      (commit! db "w1" "S1" "status" :assert "open" nil)
      {:db db :len (flen seam-log) :sha (log-sha seam-log)})))
...
(bar "seam: truncating to a recorded byte length restores the exact pre-write state"
     (let [{:keys [db len]} @seam
           before (live-triples (replay seam-log))
           _      (commit! db "w1" "S2" "status" :assert "second" nil)
...
(def fx
  (delay
    (let [db   (new-database world-log)
          root (k "version-id" nil [])
          _    (u "world-create!" db "w" "A" root)
          head0 (u "world-head" db "A")
          bid  (:ok (u "world-blob-put!" db "w" raw-a))
          cid  (:ok (u "world-begin!" db "w" "A" head0 n1))
          _    (u "world-append!" db "w" cid (k "put-op" slot-a mode bid))
          vA   (:ok (u "world-seal!" db "w" cid))
          lock (u "world-lock!" db vA build-spec)
          rcpt (:ok (u "world-build!" db "w" (:ok lock)))
          len0 (flen world-log)
          sha0 (log-sha world-log)
          prom (u "world-promote!" db "w" "A" head0 cid rcpt)]
      {:db db :log world-log :root root :head0 head0 :bid bid :cid cid :vA vA
...
(bar "fixture: the accepted promotion moved the derived head to the sealed Version"
     (let [f @fx] (= (:vA f) (:head1 f) (u "world-head" (:db f) "A"))))
...
(bar "restart: the derived head survives a cold restart (replay, nothing in memory)"
     (let [f  @fx
           db (reopen (copy-log (:log f) "restart"))]
       (= (:vA f) (u "world-head" db "A"))))
...
(bar "restart: blob bytes replay byte-identically (base64 facts, not files)"
     (let [f  @fx
           db (reopen (copy-log (:log f) "restart-blob"))]
       (java.util.Arrays/equals ^bytes raw-a ^bytes (u "world-blob" db (:bid f)))))
(bar "restart: the sealed Version record replays intact"
     (let [f  @fx
           db (reopen (copy-log (:log f) "restart-version"))]
       (= (u "world-version" (:db f) (:vA f)) (u "world-version" db (:vA f)))))
(bar "restart: the resolved manifest is identical across restart"
     (let [f  @fx
           db (reopen (copy-log (:log f) "restart-manifest"))]
       (and (= 1 (count (u "world-manifest" db (:vA f))))
            (= (u "world-manifest" (:db f) (:vA f)) (u "world-manifest" db (:vA f))))))
(bar "restart: a sealed candidate is still SEALED to the same Version after restart"
     (let [f  @fx
           db (reopen (copy-log (:log f) "restart-candidate"))]
       (= (:vA f) (:sealed (u "world-candidate" db (:cid f))))))
...
(def probe-src
  (str "(load-file \"database.clj\")\n"
       ...
       "  (println (:ok (world-lock! db head (clojure.edn/read-string spec)))))\n"))
(spit probe-path probe-src)

(bar "lock: WorldLockId is stable across an in-process cold restart"
     (let [f  @fx
           db (reopen (copy-log (:log f) "restart-lock"))]
       (= (:lock-id f) (:ok (u "world-lock!" db (u "world-head" db "A") build-spec)))))
...
(bar "lock: the rendered WorldLock contains no timestamp, pid, nonce or cid"
     (let [f   @fx
           txt (k "render-record" (:lock (u "world-lock!" (:db f) (:vA f) build-spec)))]
...
(defn unpromotable
  ...
  [db log name expected-head cid receipt]
  (let [head0 (u "world-head" db name)
        sha0  (log-sha log)
        len0  (flen log)
        r     (u "world-promote!" db "w" name expected-head cid receipt)]
    {:reject (:reject r)
     :ok (:ok r)
     :head-moved? (not= head0 (u "world-head" db name))
...
(def unsealed-fx
  (delay
    (let [f    @fx
          ...
          db   (reopen log)
          head (u "world-head" db "A")
          bid  (:ok (u "world-blob-put!" db "w" raw-b))
          cid  (:ok (u "world-begin!" db "w" "A" head n2))]
      (u "world-append!" db "w" cid (k "put-op" slot-b mode bid))
      {:db db :log log :cid cid :head head})))

(bar "unsealed: promoting an UNSEALED candidate is rejected :world-candidate-unsealed"
     (let [f @fx
           {:keys [db log cid head]} @unsealed-fx
           r (unpromotable db log "A" head cid (:receipt f))]
...
(bar "unsealed: only a SEALED candidate can become a Version"
     (let [{:keys [db cid]} @unsealed-fx]
       (nil? (:sealed (u "world-candidate" db cid)))))
...
(bar "gapped: a candidate missing an interior op record is UNPROMOTABLE"
     ...
     (let [f    @fx
           ...
           db0  (reopen log)
           head (u "world-head" db0 "A")
           bid  (:ok (u "world-blob-put!" db0 "w" raw-b))
           cid  (:ok (u "world-begin!" db0 "w" "A" head n2))
           _    (u "world-append!" db0 "w" cid (k "put-op" slot-b mode bid))
           _    (u "world-append!" db0 "w" cid (k "put-op" "src/app/x.bclj" mode bid))
           _    (u "world-append!" db0 "w" cid (k "delete-op" slot-b))
           _    (u "world-seal!" db0 "w" cid)
...
(bar "unpromotable: a candidate that never sealed yields NO Version record"
     (let [{:keys [db cid]} @unsealed-fx]
       (nil? (u "world-version" db (:sealed (u "world-candidate" db cid))))))
...
(let [pass (- @total @failures)]
  ...
  (if (zero? @failures)
    ...
    (do (println (str "world-persistence: " @failures " FAILED — section 0 pins the"
                      ...
                      " contract that database.clj + fram.world must still meet."))
        (System/exit 1))))
