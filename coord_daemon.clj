            ...
            (if (< (.length f) pre)
              ...
              (do (binding [*out* *err*]
                  ...
                  (with-open [raf (java.io.RandomAccessFile. f "rw")]
                    (.setLength raf pre)                       ; drop any torn partial batch
                    (.seek raf pre)
  ...
  (do
    ...
    (locking dlock                     ; serialize writes + short immutable captures
     (case (:op req)
      ...
      :edit-protocol {:ok true :protocol edit-protocol-name :version (current-seq @co)}
...
(defn serve-flat-daemon [port flat]
  ...
  (let [gate (fram.rt/boot-rewrite-gate! (str flat))]
    (try (recover-edit-journal! (str flat))   ; crashed :edit-commit? redo/discard the whole batch BEFORE any boot read
         (boot-flat! flat)
