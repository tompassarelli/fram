...
(defn- src-label [ctx view ^String src ents]
  ...
  (if (some? mn) mn (str/replace (str (last (str/split src #"/"))) #"\.[^.]+$" ""))))
...
(defn- callers-of [ctx view BOUND REFERS srcs ents-of defn-meta]
  ...
  (contains? #{"extend-type" "extend-protocol"} (str h)) (reduce (fn [b cc] (if (= "list" (rr/kind-of ctx view cc)) (let [mnode (nth (rr/ordered-children ctx cc) 0 nil)
