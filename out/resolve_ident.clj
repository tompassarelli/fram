...
(defn by-subject [^Graph g subject]
  (rot/occurrences (rot/by-slot0 (view g) subject)))

(defn by-subject-predicate [^Graph g subject predicate]
  (rot/occurrences (rot/by-slot01 (view g) subject predicate)))

(defn by-predicate [^Graph g predicate]
  (rot/occurrences (rot/by-slot1 (view g) predicate)))

(defn by-predicate-value [^Graph g predicate value]
  (rot/occurrences (rot/by-slot12 (view g) predicate value)))
...
(defn subject-at [^Graph g occurrence]
  ...
  (if (nil? p) nil (t/triple-slot0 p))))

(defn predicate-at [^Graph g occurrence]
  (let [p (proposition-at g occurrence)]
  (if (nil? p) nil (t/triple-slot1 p))))

(defn value-at [^Graph g occurrence]
  (let [p (proposition-at g occurrence)]
  (if (nil? p) nil (t/triple-slot2 p))))
...
(defn occurrence-order [occurrence]
  (if (t/occurrence-coordinate? occurrence) [1 (t/triple-slot2 (t/triple-slot0 occurrence)) (t/triple-slot2 occurrence)] [0 (if (integer? occurrence) occurrence 0) 0]))

(defn ^String writer-of [^Graph g occurrence]
  (if (t/occurrence-coordinate? occurrence) (let [hit (get (graph-writers g) (t/triple-slot0 occurrence))]
  (if (nil? hit) "" (str hit))) ""))
