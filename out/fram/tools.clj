...
(def Op-values #{::tell ::untell ::show ::query ::validate ::upsert-form ::set-body ::rename-def ::insert-after ::replace-in-body})
...
(defn- missing-req [op args]
  (let [need-spo (or (= op :tell) (= op :untell))
...
(defn call [claims idx cat ^String tool args]
  (let [tool (if (= tool "query") "ask" tool)
  ...
  (= op :untell) {:write {:op "retract" :l te :p pred :r (ref-value claims pred (:object args))}}
