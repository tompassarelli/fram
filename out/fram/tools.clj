...
(def Op-values #{::tell ::retract ::show ::query ::validate ::upsert-form ::set-body ::rename-def ::insert-after ::replace-in-body ::edit-transaction})
...
(defn- missing-req [op args]
  (let [need-spo (or (= op :tell) (= op :retract))
   need-module (or (= op :upsert-form) (or (= op :set-body) (or (= op :rename-def) (or (= op :insert-after) (or (= op :replace-in-body) (= op :edit-transaction))))))
   ...
   e10 (if (and (= op :edit-transaction) (nil? (:edits args))) ["missing required param 'edits'"] [])]
  (vec (concat e1 (concat e2 (concat e3 (concat e4 (concat e5 (concat e6 (concat e7 (concat e8 (concat e9 e10))))))))))))

(defn call [facts idx cat ^String tool args]
  ...
  (= op :show) {:rows (mapv (fn [c] (->PredVal (:p c) (:r c))) (k/q-by-l facts te))}
