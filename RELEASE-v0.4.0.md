...
- `bin/fram-daemon` is native-first and fails closed. The default production
  route requires a completed Beagle native artifact; it never falls back to a
  JVM silently.
- `FRAM_DAEMON_RUNTIME=graal` selects a self-contained Graal coordinator when
...
- The Cloudflare coordinator image now builds a static musl Graal executable
...
- Reachability metadata is checked in and was produced from exercised daemon
  paths. The image build is multi-stage and the final runtime contains the
  coordinator executable rather than a JVM.
...
- v0.3 flat logs are not served directly. Quiesce the old writer and run the
  ...
  wire protocol.
- Existing service definitions that relied on an implicit JVM daemon must now
