...
#define FRAM_SERVER_ADAPTER_ABI 3u
...
_Static_assert(sizeof(fram_server_store_boot_arg_3) == sizeof(native_vec *),
               "generated snapshot byte-vector ABI");
...
static int boot_generation_from_log(
  ...
  borrowed_byte_vector borrowed_snapshot;
  ...
  *generation_out = NULL;
  ...
  memset(&borrowed_snapshot, 0, sizeof(borrowed_snapshot));
  ...
  if (!borrowed_byte_vector_init(&borrowed, log_bytes, log_length, error,
                                 error_capacity) ||
      !borrowed_byte_vector_init(&borrowed_snapshot, snapshot_bytes,
                                 snapshot_length, error, error_capacity)) {
  ...
  result = FRAM_SERVER_CALL_STORE_BOOT(
      ...
      generated_space_id, &borrowed.vector, &borrowed_snapshot.vector);
...
cleanup:
  ...
  borrowed_byte_vector_destroy(&borrowed_snapshot);
  free(log_bytes);
  free(snapshot_bytes);
  owner_release(owner);
  if (status != FRAM_SERVER_OK) {
